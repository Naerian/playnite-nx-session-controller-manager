...
- **Advanced Notification Styling:** Added independent state backgrounds and borders, state-aware border gradients, border glow, per-side border thickness, icon containers, content ordering, typography, badge placement, and expanded image controls for Desktop and Fullscreen notifications.
- **Advanced Overlay Styling:** Added richer card layouts, block ordering, metadata orientation, image composition, gradients, per-side borders, border glow, controller containers, and expanded typography and badge styling.
- **Named Imported Designs:** Visual profiles can be named when exported, imported into a dedicated group, replaced by name, deleted from the selector, and used for both notifications and the disconnect overlay.
- **Creator Documentation:** Added complete English and Spanish authoring references, generic preview commands, property tables, manifests, examples, contribution guidance, compatibility rules, and troubleshooting.
- **Safe Removed-Pack Fallback:** If a creator or imported design is no longer available, its materialized appearance is preserved as Custom instead of being replaced by an unrelated plugin preset.
